...
function formatZonedTime(now: Date, timeZone: string | undefined): string | null {
 ...
 * exceeds 12.
 */
function offsetDeltaLabel(now: Date, timeZone: string | undefined): string | null {
  if (!timeZone) return null;
  ...
  const magnitude = Number.isInteger(hours) ? String(hours) : hours.toFixed(2).replace(/0+$/, '');
...
function useZonedTime(timeZone: string | undefined): string | null {
  ...
  return useSyncExternalStore(subscribe, () => formatZonedTime(new Date(), timeZone));
}

/**
 * One quiet practical row for a destination: "Local time 14:32 (+2 h) ·
 * 1 EUR ≈ 26.4 CZK". Each half earns its place - the clock only shows when
 * the destination's offset differs from the device's, the rate only when the
 * currencies differ and Frankfurter quotes the pair. Nothing interesting
 * (same zone, same currency) renders nothing; silence is the default.
 */
export function PracticalInfo({ place, timezone }: PracticalInfoProps) {
  const userCurrency = useAppStore((s) => s.currency);
  const placeCurrency = destinationCurrency(codeOf(place));
  ...
  if (!showTime && rate === null) return null;

  return (
    <section className="practical-info" aria-label="Local time and currency">
      {showTime && (
      ...
      {showTime && rate !== null && (
        <span className="practical-sep" aria-hidden>
          ·
        </span>
      )}
      {rate !== null && (
